...
public class Tab3 extends HttpSupportActivity {
	...
	public void btn_Serache(View v) {
		startActivityForResult(new Intent(context, SeracheView.class),
				RequestCode);
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode==RequestCode) {
			if (data!=null) {
				ToastUtil("收到请求码为:"+data.getStringExtra("s_msg"));
			}else {
				ToastUtil("未作有效操作");
			}
		}else {
	...
	class Tab3Adapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = View.inflate(context, R.layout.tab1_page_cell,
						null);
			}
			return convertView;
		}
